...
export default function Datetime({ pubDatetime, modDatetime, size = "sm", className }: Props) {
  ...
  return (
    <div className={`flex items-center space-x-2 opacity-80 ${className}`}>
      ...
      <div className={`flex w-full justify-between italic ${size === "sm" ? "text-sm" : "text-base"}`}>
        <span>
...
const FormattedDatetime = (props: DatetimesProps) => {
  ...
  const date = myDatetime.toLocaleDateString(i18n.language, {
    year: "numeric",
    month: "short",
    day: "numeric",
  });

  const time = myDatetime.toLocaleTimeString(i18n.language, {
    hour: "2-digit",
    minute: "2-digit",
  });

  // throw new Error(`i18n.language: ${i18n.language}, date: ${date}, time: ${time}`);
  return (
    <>
